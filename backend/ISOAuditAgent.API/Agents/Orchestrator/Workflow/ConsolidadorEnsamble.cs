...
public static class ConsolidadorEnsamble
{
        ...
        HallazgosClasificados clasificacion)
    {
        ...
        foreach (var artefacto in documentos.Artefactos)
        {
            ...
            artefactosEvaluados.Add(new ArtefactoEvaluadoResultado(
                ...
                EstadoAplicacionTailoring: MapearEstadoTailoring(artefacto.EstadoTailoring),
        ...
        IReadOnlyList<HallazgoClasificado> hallazgos)
    {
        ...
        switch (artefacto.EstadoTailoring)
        {
            case EstadoTailoring.NoAplica:
            ...
            case EstadoTailoring.SinDeclararEnTailoring:
                return ResultadoEvaluacion.NoConforme; // tailoring incompleto

            case EstadoTailoring.Aplica:
            ...
            default:
                throw new EnsambleAuditoriaException(
                    $"EstadoTailoring no contemplado: {artefacto.EstadoTailoring}.");
        }
    }
    ...
    /// <summary>
    /// Mapea EstadoTailoring (contrato 2) a EstadoAplicacionTailoring
    /// (contrato 5). Mismos 3 valores; el contrato 5 los preserva para
    /// trazabilidad histórica en BD.
    /// </summary>
    private static EstadoAplicacionTailoring MapearEstadoTailoring(
        EstadoTailoring estado)
        => estado switch
        {
            EstadoTailoring.Aplica
                => EstadoAplicacionTailoring.Aplica,
            EstadoTailoring.NoAplica
                => EstadoAplicacionTailoring.NoAplica,
            EstadoTailoring.SinDeclararEnTailoring
                => EstadoAplicacionTailoring.SinDeclararEnTailoring,
            _ => throw new EnsambleAuditoriaException(
                $"EstadoTailoring no contemplado: {estado}.")
        };
}
